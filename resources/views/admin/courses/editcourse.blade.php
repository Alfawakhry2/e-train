    ...
    <div class="container py-4 fs-3">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h2 class="mb-0"><i class="bi bi-plus-circle me-2"></i>Edit Course</h2>
            </div>
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="title" type="text" class="form-control fs-3" id="title"
                            placeholder="Enter course title" value="{{ $course->title }}" required>
                    </div>
                    @error('title')
                    ...
                    <div class="mb-4">
                        ...
                        <input name="code" type="text" class="form-control fs-3" id="code"
                            placeholder="e.g. WEB-101" value="{{ $course->code }}">
                    </div>
                    @error('code')
                        <p class="my-1 text-start text-danger">*{{ $message }}</p>
                    @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <select name="trainer_id" class="form-select fs-3" id="trainer" required>
                            ...
                            <option value="{{ $course->trainer_id }}" selected>{{ $course->trainer->user->name }} =>
                                {{ $course->trainer->spec }}</option>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input name="end_date" type="date" class="form-control fs-3" id="end_date" value="{{ $course->end_date }}" disabled>
                        </div>
                    </div>
                    @error('start_date')
                        <p class="my-1 text-start text-danger">*{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="small_desc" class="form-control fs-3" id="short_desc" rows="2" placeholder="Brief description">{{ $course->small_desc }}</textarea>
                    </div>
                    @error('small_desc')
                        <p class="my-1 text-start text-danger">*{{ $message }}</p>
                    @enderror
                    <div class="mb-4">
                        ...
                        <textarea name="desc" class="form-control fs-3" id="full_desc" rows="4" placeholder="Detailed description">{{ $course->desc }}</textarea>
                    </div>
                    @error('desc')
                        <p class="my-1 text-start text-danger">*{{ $message }}</p>
                    @enderror
                    <!-- Image Upload -->
                    <div>
                        Old Image <img src="{{ asset("Storage/$course->image") }}" style="height:100px;" alt="">
                    </div>
                    <br>
                    <div class="mb-4">
