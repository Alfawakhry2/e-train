...
@section('content')
            ...
            <tbody class="fs-5">
                @foreach ($students as $student)
                    <tr>
                        ...
                        <td class="py-3 px-4 text-break">{{ $student->email }}</td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4">
                            <div class="d-flex flex-column">
                                <!-- Show button (bigger) -->
                                <div class="mb-2">
                                    <a href="{{ url("admin/show/member/$student->id") }}"
                                        class="btn btn-dark btn-sm w-100 fs-3">Show</a>
                                </div>

                                <!-- Edit and Delete buttons (beside each other) -->
                                <div class="d-flex justify-content-between">
                                    {{-- i get user with role student as $student ... and the student has all user attr --}}
                                    <a href="{{ url("admin/edit/member/$student->id") }}"
                                        class="btn btn-outline-primary btn-sm w-45 fs-3">Edit</a>
    ...
    <div class="mb-2 text-dark">
        <div class="w-50 fs-3">{{ $students->links() }}</div>
    </div>
@endsection
